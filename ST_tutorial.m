...
figure(1); imshow(uint8(in2d)); title('the image in which we insert watermark');
dost2In2d = ST.dost2(in2d);
figure(2); imshow(uint8(dost2In2d)); title('the image in which we insert watermark dost');
 %dost2In2d=real(dost2In2d ); 
J1=imnoise(dost2In2d,'gaussian',0, 0.001);
recdostIn2d = ST.idost2(ST.dost2(J1));
recdostIn2d=real(recdostIn2d);
figure(3); imshow(recdostIn2d,[]); title('the image in which we insert watermark idost');
imshow(recdostIn2d);
figure
subplot(1, 3, 1)
imagesc(in2d)
