...
dost2=uint8(dost2);
 ...
 J=imnoise(I_1,'gaussian',0, 0.1);
%   J=imnoise(dost2,'salt & pepper',0.1);
% J = histeq(I_1);
% J=uint8(J);
...
figure(7); imshow((recdost)); title('the image in which we insert watermark idost');
...
WMy=Uw*Swrec*Vw';
WMy=double(WMy);
figure(8); imshow((WMy));title('Extracted Watermark');
 WMy=1-WMy;
X11=corr2(WMy,I1_w);
I_1=-I_1;
s=psnr(I,I_1);